...
def get_long_description(here):
    ...
    try:
        with open(os.path.join(here, "README.rst"), "r", encoding="utf-8") as f:
            long_description = "teachcompute:" + f.read().split("teachcompute:")[1]
    except FileNotFoundError:
        long_description = ""
...
def get_version_str(here, default_version):
    ...
    with open(os.path.join(here, "teachcompute/__init__.py"), "r") as f:
        line = [
            _
...
class cmake_build_class_extension(Command):
    ...
    def add_missing_files(self):
        ...
        conf = os.path.join(thisdir, "teachcompute", "_config.py")
        if not os.path.exists(conf):
            print("-- setup: add empty file 'teachcompute/_config.py")
            with open(conf, "w"):
                pass
    ...
    def final_verifications(self):
        ...
        files = ["_config.py"]
        for name in files:
            full = os.path.join(thisdir, "teachcompute", name)
...
def get_ext_modules():
    ...
    ext_modules = [
        ...
        CMakeExtension(
            "teachcompute.validation.cython.direct_blas_lapack_cy",
...
def get_package_data():
    ...
    return {
        "teachcompute": known_extensions,
        "teachcompute.cpp": known_extensions,
        "teachcompute.include": known_extensions,
