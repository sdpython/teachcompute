...
PYBIND11_MODULE(cuda_gemm, m) {
  m.doc() =
      ...
      "C++ experimental implementations with CUDA and GEMM."
#else
      R"pbdoc(C++ experimental implementations with CUDA and GEMM.)pbdoc"
#endif
      ;
  m.def(
      "matmul_v1_cuda",
      ...
      py::arg("transB") = false,
      R"pbdoc(Matrix Matrix Naive Implementation on CUDA.
...
:param transB: B needs to be transposed?
  ...
  m.def(
      ...
      py::arg("n_cols2"), py::arg("B"), py::arg("C"), py::arg("transA") = false,
      py::arg("transB") = false,
      R"pbdoc(Matrix Matrix Less Naive Implementation on CUDA.
