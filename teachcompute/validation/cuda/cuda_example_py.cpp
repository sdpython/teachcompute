...
#define py_array_float16                                                       \
  py::array_t<short, py::array::c_style | py::array::forcecast>
...
:return: sum
  ...
  m.def(
      "measure_vector_add_half",
      [](const py_array_float16 &v1, const py_array_float16 &v2,
         int cuda_device, int repeat) -> py::dict {
        if (v1.size() != v2.size()) {
          throw std::runtime_error(
              "Vectors v1 and v2 have different number of elements.");
        }
        auto ha1 = v1.request();
        short *ptr1 = reinterpret_cast<short *>(ha1.ptr);
        auto ha2 = v2.request();
        short *ptr2 = reinterpret_cast<short *>(ha2.ptr);

        std::vector<int64_t> shape(v1.ndim());
        for (int i = 0; i < v1.ndim(); ++i) {
          shape[i] = v1.shape(i);
        }
        py_array_float16 result = py::array_t<short>(shape);
        py::buffer_info br = result.request();

        short *pr = static_cast<short *>(br.ptr); // pointer on result data
        if (ptr1 == nullptr || ptr2 == nullptr || pr == nullptr) {
          throw std::runtime_error("One vector is empty.");
        }
        auto tu = measure_vector_add_half(v1.size(), ptr1, ptr2, pr,
                                          cuda_device, repeat);

        py::dict res;
        res["value"] = result;
        res["half"] = std::get<0>(tu);
        res["half2"] = std::get<1>(tu);
        return res;
      },
      py::arg("v1"), py::arg("v2"), py::arg("cuda_device") = 0,
      py::arg("repeat") = 1,
...
:param repeat: number of times to repeat the addition
:param cuda_device: device id (if mulitple one)
:return: a dictionary with the result of the addition of the two arrays,
  and the time it took with half and half2
)pbdoc");
}
