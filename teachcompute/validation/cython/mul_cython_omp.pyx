...
cdef int _dmul_cython_omp01(const double* va, const double* vb, double* res,
                            Py_ssize_t p, Py_ssize_t ni, Py_ssize_t nj,
                            Py_ssize_t nk) nogil:
    cdef Py_ssize_t j, k
    for j in range(0, nj):
        res[p * nj + j] = 0
...
cdef int _dmul_cython_omp(const double* va, const double* vb, double* res,
    ...
    See
    `mul_cython_omp.pyx
    <https://github.com/sdpython/teachcompute/blob/main/teachcompute/validation/cython/mul_cython_omp.pyx>`_.
    """
    cdef double s
...
cdef int _dmul_cython_omp_t(const double* va, const double* vb, double* res,
    ...
    `mul_cython_omp_.cpp
    <https://github.com/sdpython/teachcompute/blob/main/teachcompute/validation/cython/mul_cython_omp_.cpp>`_,
    `mul_cython_omp.pyx
    <https://github.com/sdpython/teachcompute/blob/main/teachcompute/validation/cython/mul_cython_omp.pyx>`_.
    """
    cdef Py_ssize_t p, i, j, k
    if parallel == 0:
        if algo == 0:
            for i in range(0, ni):
                for j in range(0, nj):
                    res[i * nj + j] = vector_ddot_product_pointer16_sse(
                        &va[i * nk], &vb[j * nk], nk)
            return 1
    ...
    if parallel == 1:
        if algo == 0:
            for p in prange(0, ni):
                for j in range(0, nj):
                    res[p * nj + j] = vector_ddot_product_pointer16_sse(
                        &va[p * nk], &vb[j * nk], nk)
        ...
        if algo == 1:
            for p in prange(0, nj):
                for i in range(0, ni):
                    res[i * nj + p] = vector_ddot_product_pointer16_sse(
                        &va[i * nk], &vb[p * nk], nk)
